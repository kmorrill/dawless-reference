...
| **playMode** | string | No | Playback behavior: `"loop"` (default) repeats after the last bar; `"playThrough"` plays once then stops scheduling notes. |
...
Provides per‑project settings for MIDI output and parameter naming.

| Key | Type | Required | Description |
| :---- | :---- | :---- | :---- |
| **portName** | string | No | MIDI port name to send messages to (e.g., `"OP-XY"`). |
| **device** | string | No | Default device id for resolving `name:<id>` CC destinations (e.g., `"op-xy"`, `"polyend_synth"`). Tracks may override via `track.device`. |
| **ccNameMap** | object | No | Optional string→integer\[0..127\] map to extend/override `name:<id>` CC destinations globally. |
...
| **type** | string | Yes | Device synth engine / instrument type hint (e.g., OP‑XY `"sampler"`, `"axis"`, Polyend Synth `"ACD"`, `"WAVS"`). |
| **midiChannel** | integer 0–15 | Yes | MIDI channel for this track. |
| **portName** | string | No | MIDI port name override for this track (defaults to `deviceProfile.portName`). |
| **device** | string | No | Device id override for resolving `name:<id>` CC destinations for this track. |
...
| **ccNameMap** | object | No | Optional string→integer\[0..127\] CC map overlay for this track. |
| **program** | integer \| object | No | Optional Program Change to send on `start()` (int 0–127 or `{program:number, bankMsb?, bankLsb?}`). |
| **programChanges** | array | No | Optional scheduled Program Change events (objects with `{t, program}`). |

> *Note:* There is no `muted` field in this schema; omit a track to silence it.
...
### 4.2 Program changes (optional)

Use `program` and/or `programChanges` to select presets on external devices.

- `program`: either an integer **0–127** *or* an object:
  - `program` (or `number`): integer **0–127**
  - `bankMsb` (optional): integer **0–127** (sends CC0)
  - `bankLsb` (optional): integer **0–127** (sends CC32)
- `programChanges`: array of objects with:
  - `t`: time object (`{ticks:n}` or `{bar:n, step:n}`)
  - `program`: same format as `program`
